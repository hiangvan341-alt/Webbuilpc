...
import { CheckCircle2, ChevronRight, FileSpreadsheet, LogIn, LogOut, Plus, Printer, RotateCcw, Save, Share2, ShieldCheck, ShoppingCart, Upload, UserPlus, X } from 'lucide-react'
...
export default function App(){
  ...
  return <div className="app">
    <header className="topbar">
      ...
      <nav><a href="#builder">Build PC</a><a href="#guide">Hướng dẫn</a><button className="admin" onClick={()=>setAdminOpen(true)}><LogIn size={16}/> Quản trị</button></nav>
    </header>
    ...
    {active&&<div className="modal-backdrop" onClick={()=>setActive(null)}><div className="modal" onClick={e=>e.stopPropagation()}><div className="modal-head"><div><h2>{categories.find(c=>c.key===active)?.label}</h2><p>{categoryProducts.length} sản phẩm đã được phân loại vào đúng danh mục</p></div><button onClick={()=>setActive(null)}><X/></button></div>
      ...
      <div className="filter-result">Đang hiển thị <b>{filtered.length}</b> / {categoryProducts.length} sản phẩm</div><div className="product-grid">{filtered.map(p=><article className="product" key={p.id}><img src={p.image}/><div className={'stock '+(p.stock<=0?'out':'')}>{p.stock>0?`Còn ${p.stock}`:'Hết hàng'}</div><div className="group-tag">{p.group_name || categories.find(c=>c.key===p.category)?.shortLabel}</div><h3>{p.name}</h3><p>{p.warranty || 'Bảo hành: đang cập nhật'}</p><strong>{money(p.price)}</strong><button disabled={p.stock<=0} onClick={()=>choose(p)}>{p.stock>0?'Chọn sản phẩm':'Tạm hết hàng'}</button></article>)}</div>{!filtered.length&&<div className="empty">Không có sản phẩm phù hợp với bộ lọc hiện tại.</div>}</div></div>}
    {adminOpen&&<AdminPanel onClose={()=>setAdminOpen(false)} onImported={()=>getProducts<any[]>().then(rows=>setProducts(rows.map(dbToProduct).sort((a,b)=>categoryOrder[a.category]-categoryOrder[b.category] || a.name.localeCompare(b.name,'vi'))))}/>} 
    {quoteOpen&&<Quote build={build} total={total} onClose={()=>setQuoteOpen(false)}/>} 
  </div>
}

function AdminPanel({onClose,onImported}:{onClose:()=>void,onImported:()=>void}){
  ...
  return <div className="modal-backdrop"><div className="admin-panel"><div className="modal-head"><div><h2>Quản trị NOVA TECH PC</h2><p>Nhập sản phẩm Excel và quản lý tài khoản admin</p></div><button onClick={onClose}><X/></button></div>
    {!token?<form className="login-form" onSubmit={login}><label>Tên đăng nhập<input value={username} onChange={e=>setUsername(e.target.value)} required/></label><label>Mật khẩu<input type="password" value={password} onChange={e=>setPassword(e.target.value)} required/></label><button className="primary"><LogIn size={18}/> Đăng nhập</button><small>Tài khoản ban đầu: <b>admin</b> • Mật khẩu: <b>Do12345</b>. Nên tạo tài khoản mới và đổi thông tin sau lần đăng nhập đầu tiên.</small></form>:<div className="admin-body">
      ...
      <section className="admin-card"><h3><UserPlus size={19}/> Tạo tài khoản admin</h3><form className="account-form" onSubmit={createAccount}><input placeholder="Tên tài khoản" value={newUser} onChange={e=>setNewUser(e.target.value)} minLength={3} required/><input type="password" placeholder="Mật khẩu (tối thiểu 6 ký tự)" value={newPass} onChange={e=>setNewPass(e.target.value)} minLength={6} required/><button><Plus size={17}/> Tạo tài khoản</button></form></section>
    </div>}
    {status&&<div className="admin-status">{status}</div>}
  </div></div>
}

function Quote({build,total,onClose}:{build:BuildSelection,total:number,onClose:()=>void}){
  ...
  return <div className="modal-backdrop quote-backdrop"><div className="quote-shell"><div className="quote-actions"><button onClick={onClose}><X size={17}/> Đóng</button><button className="primary-inline" onClick={()=>window.print()}><Printer size={17}/> In báo giá</button></div><article className="quotation">
    <header className="quote-header"><div><h3>CÔNG TY TNHH CÔNG NGHỆ NOVA TECH PC</h3><p><b>Trụ sở:</b> 134 Nguyễn Chánh - Hoàng Mai - HN</p><p><b>Hotline:</b> 0377 455 855 &nbsp; | &nbsp; <b>Email:</b> contact@novatechpc.vn</p><p><b>Website:</b> Novatechpc.vn</p></div><img src="/nova-tech-logo.png"/></header><div className="green-line"/><h1>BÁO GIÁ SẢN PHẨM</h1><div className="quote-meta"><span>Ngày báo giá: <b>{today()}</b></span><span>Đơn vị tính: <b>VNĐ</b></span></div>
    ...
    <p className="customer-note"><b>Quý khách lưu ý:</b> Giá bán, khuyến mãi của sản phẩm và tình trạng còn hàng có thể thay đổi bất cứ lúc nào mà không kịp báo trước.</p><footer><span>Để biết thêm chi tiết, vui lòng liên hệ<br/><b>Hotline: 0377 455 855</b> (8h00 - 21h00 hàng ngày)</span><b>NOVA TECH PC CHÂN THÀNH CẢM ƠN QUÝ KHÁCH</b></footer>
  </article></div></div>
}
