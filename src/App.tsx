...
import { CheckCircle2, ChevronRight, FileSpreadsheet, Grid2X2, List, LogIn, LogOut, Plus, Printer, Rows3, RotateCcw, Save, Share2, ShieldCheck, ShoppingCart, Upload, User, UserPlus, X } from 'lucide-react'
...
function AdminPanel({onClose,onImported,quoteSettings,onQuoteSaved}:{onClose:()=>void,onImported:()=>void,quoteSettings:QuoteSettings,onQuoteSaved:(v:QuoteSettings)=>void}){
  ...
  async function createCustomer(e:FormEvent){e.preventDefault();setStatus('Đang tạo tài khoản người dùng...');try{await rpc('admin_create_user_account',{p_token:token,p_username:newCustomer,p_password:newCustomerPass});setStatus(`Đã tạo tài khoản người dùng ${newCustomer}.`);setNewCustomer('');setNewCustomerPass('')}catch(err){setStatus('Không tạo được tài khoản người dùng: '+String(err))}}
...
<section className="admin-card"><h3><UserPlus size={19}/> Tạo tài khoản người dùng</h3><p>Admin chỉ đặt tên đăng nhập và mật khẩu mới. Mật khẩu không hiển thị lại và chỉ được lưu dưới dạng mã hóa.</p><form className="account-form" onSubmit={createCustomer}><input autoComplete="off" placeholder="Tên tài khoản người dùng" value={newCustomer} onChange={e=>setNewCustomer(e.target.value)} minLength={3} required/><input type="password" autoComplete="new-password" placeholder="Mật khẩu (tối thiểu 6 ký tự)" value={newCustomerPass} onChange={e=>setNewCustomerPass(e.target.value)} minLength={6} required/><button><Plus size={17}/> Tạo người dùng</button></form></section>
