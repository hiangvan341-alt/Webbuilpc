...
export default function App(){
  ...
  return <div className="app">
    <header className="topbar">
      ...
      <nav><a href="#builder">Build PC</a><a href="#guide">Hướng dẫn</a><button className="user-login" onClick={()=>setUserOpen(true)} aria-label="Đăng nhập tài khoản khách hàng"><span className="user-login-icon"><User size={17}/></span><span className="user-login-text"><b>Đăng nhập</b><small>Khách hàng</small></span></button><button className="admin" onClick={()=>setAdminOpen(true)}><LogIn size={16}/> Quản trị</button></nav>
    </header>
...
function UserLogin({onClose}:{onClose:()=>void}){
  ...
  async function login(e:FormEvent){e.preventDefault();setStatus('Đang đăng nhập...');try{const r=await rpc<{token:string}[]>('user_login',{p_username:username.trim(),p_password:password});if(!r?.[0]?.token)throw new Error('Sai tài khoản hoặc mật khẩu');localStorage.setItem('nova-user-token',r[0].token);if(remember)localStorage.setItem('nova-user-name',username);else localStorage.removeItem('nova-user-name');setPassword('');setStatus('Đăng nhập thành công.')}catch(err){setStatus('Đăng nhập thất bại: '+String(err))}}
  return <div className="modal-backdrop customer-login-backdrop"><div className="customer-login-dialog"><button className="customer-login-close" onClick={onClose} aria-label="Đóng"><X/></button><section className="customer-login-brand"><img src="/nova-tech-logo.png"/><span className="customer-login-eyebrow">NOVA TECH PC</span><h2>Chào mừng bạn quay lại</h2><p>Đăng nhập để lưu cấu hình, xem lại báo giá và tiếp tục lựa chọn linh kiện trên mọi thiết bị.</p><div className="customer-login-points"><span><CheckCircle2 size={17}/> Lưu cấu hình đang chọn</span><span><CheckCircle2 size={17}/> Quản lý báo giá cá nhân</span><span><CheckCircle2 size={17}/> Dữ liệu đồng bộ an toàn</span></div></section><form className="customer-login-form" onSubmit={login}><div className="customer-login-title"><User size={22}/><div><b>Đăng nhập khách hàng</b><small>Tài khoản do NOVA TECH PC cấp</small></div></div><label>Tên đăng nhập<div className="login-input-wrap"><User size={18}/><input autoFocus autoComplete="username" placeholder="Nhập tên tài khoản" value={username} onChange={e=>setUsername(e.target.value)} required/></div></label><label>Mật khẩu<div className="login-input-wrap"><KeyRound size={18}/><input type="password" autoComplete="current-password" placeholder="Nhập mật khẩu" value={password} onChange={e=>setPassword(e.target.value)} required/></div></label><label className="remember-login customer-remember"><input type="checkbox" checked={remember} onChange={e=>setRemember(e.target.checked)}/> Ghi nhớ tên đăng nhập</label><button className="customer-login-submit"><LogIn size={18}/> Đăng nhập</button><p className="customer-login-help">Chưa có tài khoản hoặc quên mật khẩu? Liên hệ Hotline <b>0377 455 855</b>.</p>{status&&<div className="admin-status">{status}</div>}</form></div></div>
}
