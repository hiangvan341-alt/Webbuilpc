...
type QuoteHistoryRow={id:string;quote_code:string;customer_name:string;customer_phone:string;customer_address:string;customer_note:string;total:number;created_at:string;items:any[]}
...
export default function App(){
  ...
  return <div className="app">
    <header className="topbar">
      ...
      <nav><a href="#builder">Build PC</a><a href="#guide">Hướng dẫn</a>{employee?<><button className="employee-history-btn" onClick={()=>setHistoryOpen(true)}><Clock3 size={16}/> Lịch sử báo giá</button><button className="user-login logged-in" onClick={()=>{localStorage.removeItem('nova-user-token');localStorage.removeItem('nova-user-session-name');setEmployee(null)}} title="Bấm để đăng xuất"><span className="user-login-icon"><User size={17}/></span><span className="user-login-text"><b>{employee.username}</b><small>Nhân viên • Đăng xuất</small></span></button></>:<button className="user-login" onClick={()=>setUserOpen(true)} aria-label="Đăng nhập tài khoản nhân viên"><span className="user-login-icon"><User size={17}/></span><span className="user-login-text"><b>Đăng nhập</b><small>Nhân viên</small></span></button>}<button className="admin" onClick={()=>setAdminOpen(true)}><LogIn size={16}/> Quản trị</button></nav>
    </header>
...
function UserLogin({onClose,onLoggedIn}:{onClose:()=>void,onLoggedIn:(v:EmployeeSession)=>void}){
  ...
  return <div className="modal-backdrop customer-login-backdrop"><div className="customer-login-dialog"><button className="customer-login-close" onClick={onClose} aria-label="Đóng"><X/></button><section className="customer-login-brand"><img src="/nova-tech-logo.png"/><span className="customer-login-eyebrow">NOVA TECH PC</span><h2>Khu vực nhân viên báo giá</h2><p>Đăng nhập để lập báo giá cho khách hàng và lưu toàn bộ lịch sử theo tài khoản nhân viên.</p><div className="customer-login-points"><span><CheckCircle2 size={17}/> Lưu từng báo giá khách hàng</span><span><CheckCircle2 size={17}/> Tìm lại theo tên hoặc số điện thoại</span><span><CheckCircle2 size={17}/> Mỗi nhân viên có lịch sử riêng</span></div></section><form className="customer-login-form" onSubmit={login}><div className="customer-login-title"><User size={22}/><div><b>Đăng nhập nhân viên</b><small>Tài khoản do Admin NOVA TECH PC cấp</small></div></div><label>Tên đăng nhập<div className="login-input-wrap"><User size={18}/><input autoFocus autoComplete="username" placeholder="Nhập tên tài khoản" value={username} onChange={e=>setUsername(e.target.value)} required/></div></label><label>Mật khẩu<div className="login-input-wrap"><KeyRound size={18}/><input type="password" autoComplete="current-password" placeholder="Nhập mật khẩu" value={password} onChange={e=>setPassword(e.target.value)} required/></div></label><label className="remember-login customer-remember"><input type="checkbox" checked={remember} onChange={e=>setRemember(e.target.checked)}/> Ghi nhớ tên đăng nhập</label><button className="customer-login-submit"><LogIn size={18}/> Đăng nhập làm việc</button>{status&&<div className="admin-status">{status}</div>}</form></div></div>
}
...
function Quote({settings,build,total,employee,onNeedLogin,onClose}:{settings:QuoteSettings,build:BuildSelection,total:number,employee:EmployeeSession|null,onNeedLogin:()=>void,onClose:()=>void}){
  ...
  async function saveQuote(printAfter=false){if(!employee){onNeedLogin();return}if(!customerName.trim()){setStatus('Vui lòng nhập tên khách hàng.');return}setStatus('Đang lưu báo giá...');try{const payload=items.map(p=>({product_id:p.id,category:p.category,name:p.name,warranty:p.warranty,quantity:1,unit_price:p.price,line_total:p.price}));const r=await rpc<{quote_code:string}[]>('employee_save_quote',{p_token:employee.token,p_customer_name:customerName.trim(),p_customer_phone:customerPhone.trim(),p_customer_address:customerAddress.trim(),p_customer_note:customerNote.trim(),p_items:payload,p_total:total,p_quote_settings:settings});const code=r?.[0]?.quote_code||'';setSavedCode(code);setStatus('Đã lưu báo giá '+code+' vào lịch sử của '+employee.username+'.');if(printAfter)setTimeout(()=>window.print(),150)}catch(err){setStatus('Không lưu được báo giá: '+String(err))}}
  return <div className="modal-backdrop quote-backdrop"><div className="quote-shell"><div className="quote-actions"><button onClick={onClose}><X size={17}/> Đóng</button>{employee?<><button onClick={()=>saveQuote(false)}><Save size={17}/> Lưu báo giá</button><button className="primary-inline" onClick={()=>saveQuote(true)}><Printer size={17}/> Lưu & In</button></>:<button className="primary-inline" onClick={onNeedLogin}><LogIn size={17}/> Đăng nhập nhân viên để lưu</button>}</div><div className="quote-customer-form"><label>Tên khách hàng *<input value={customerName} onChange={e=>setCustomerName(e.target.value)} placeholder="Ví dụ: Nguyễn Văn A"/></label><label>Số điện thoại<input value={customerPhone} onChange={e=>setCustomerPhone(e.target.value)} placeholder="09..."/></label><label>Địa chỉ<input value={customerAddress} onChange={e=>setCustomerAddress(e.target.value)} placeholder="Địa chỉ giao hàng"/></label><label>Ghi chú<input value={customerNote} onChange={e=>setCustomerNote(e.target.value)} placeholder="Nhu cầu, thời gian hẹn..."/></label></div>{status&&<div className="quote-save-status">{status}</div>}<article className="quotation">
    <header className="quote-header"><div><h3>{settings.company_name}</h3><p><b>Trụ sở:</b> {settings.address}</p><p><b>Hotline:</b> {settings.hotline} &nbsp; | &nbsp; <b>Email:</b> {settings.email}</p><p><b>Website:</b> {settings.website}</p></div><img src="/nova-tech-logo.png"/></header><div className="green-line"/><h1>BÁO GIÁ SẢN PHẨM</h1><div className="quote-meta"><span>Ngày báo giá: <b>{today()}</b></span><span>{savedCode&&<>Mã: <b>{savedCode}</b> • </>}Nhân viên: <b>{employee?.username||'Chưa đăng nhập'}</b></span><span>Đơn vị tính: <b>VNĐ</b></span></div>{customerName&&<div className="quote-customer"><b>Khách hàng: {customerName}</b>{customerPhone&&<span> • SĐT: {customerPhone}</span>}{customerAddress&&<span> • Địa chỉ: {customerAddress}</span>}</div>}
...
function QuoteHistory({employee,onClose}:{employee:EmployeeSession,onClose:()=>void}){
  const [rows,setRows]=useState<QuoteHistoryRow[]>([]);const [search,setSearch]=useState('');const [status,setStatus]=useState('Đang tải lịch sử...');const [selected,setSelected]=useState<QuoteHistoryRow|null>(null)
  ...
  useEffect(()=>{load()},[])
  return <div className="modal-backdrop"><div className="history-panel"><div className="modal-head"><div><h2>Lịch sử báo giá</h2><p>Nhân viên: <b>{employee.username}</b> • Chỉ hiển thị các báo giá do tài khoản này tạo</p></div><button onClick={onClose}><X/></button></div><div className="history-tools"><div><Search size={17}/><input value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==='Enter'&&load()} placeholder="Tìm tên khách, số điện thoại hoặc mã báo giá"/></div><button onClick={load}><Search size={16}/> Tìm</button></div>{status&&<div className="admin-status">{status}</div>}<div className="history-table"><table><thead><tr><th>Mã báo giá</th><th>Khách hàng</th><th>Số điện thoại</th><th>Tổng tiền</th><th>Ngày tạo</th><th></th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><b>{r.quote_code}</b></td><td>{r.customer_name}</td><td>{r.customer_phone||'-'}</td><td>{money(Number(r.total))}</td><td>{new Date(r.created_at).toLocaleString('vi-VN')}</td><td><button onClick={()=>setSelected(r)}>Xem</button></td></tr>)}</tbody></table></div>{selected&&<div className="history-detail"><div><h3>{selected.quote_code} — {selected.customer_name}</h3><button onClick={()=>setSelected(null)}><X size={16}/></button></div><p>{selected.customer_phone&&<>SĐT: <b>{selected.customer_phone}</b> • </>}{selected.customer_address}</p><ul>{(selected.items||[]).map((x:any,i:number)=><li key={i}><span>{x.name}</span><b>{money(Number(x.line_total||x.unit_price||0))}</b></li>)}</ul><strong>Tổng: {money(Number(selected.total))}</strong></div>}</div></div>
}
