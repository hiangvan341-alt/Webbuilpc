-- MODULE 10: Phân quyền Admin chính / Admin phụ - v2.0.4
-- Có thể chạy lặp lại, không xóa tài khoản hoặc dữ liệu hiện có.
...
alter table public.admin_accounts
  add column if not exists role text not null default 'sub_admin';

update public.admin_accounts
set role = case
  when lower(trim(username)) = 'admin' then 'super_admin'
...
alter table public.admin_accounts
  add constraint admin_accounts_role_check check (role in ('super_admin','sub_admin'));
...
-- Trả về role để frontend ẩn chức năng tạo Admin đối với Admin phụ.
...
-- Chỉ Admin chính được tạo Admin phụ.
...
begin
  if not public.is_super_admin(p_token) then
    raise exception 'Chỉ Admin chính mới được tạo tài khoản Admin phụ';
  end if;
  if v_username !~ '^[a-z0-9_.-]{3,40}$' then
    raise exception 'Tên tài khoản không hợp lệ';
  end if;
  ...
  values(v_username,extensions.crypt(p_password,extensions.gen_salt('bf',12)),'sub_admin',true);
exception when unique_violation then
  raise exception 'Tên tài khoản Admin đã tồn tại';
end
$$;
