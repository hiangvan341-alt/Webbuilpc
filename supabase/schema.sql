...
values ('admin', crypt('Do12345', gen_salt('bf', 12)))
...
begin
  ...
  where lower(username)=lower(trim(p_username)) and is_active and password_hash=crypt(p_password,password_hash);
...
begin
  ...
  insert into admin_accounts(username,password_hash) values(trim(p_username),crypt(p_password,gen_salt('bf',12)));
end $$;
